...
is_proc=`nsls /castor/cern.ch/user/s/sviret/CMS/Extraction/${2}/ | grep ${2}_${3} | wc -l`
...
OUTPUT_NAME=Extracted_${2}_${3}.root
...
if [ "$ntots" = "$nprocfiles" ]; then
    xrdcp extracted.root root://castorcms/$CASTORHOME/CMS/Extraction/${2}/$OUTPUT_NAME
else
    mutt -s '[Extractor]:Run '${2}_${3}' problematic: '$nprocfiles'/'$ntots viret@in2p3.fr < /dev/null
fi
