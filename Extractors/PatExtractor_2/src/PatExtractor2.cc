...
void PatExtractor2::beginRun(Run const& run, EventSetup const& setup) 
{
  ...
  {    
	...
	std::cout << "Processing " << i << "th event" << std::endl;
      ...
      PatExtractor2::doAna();   // Then do the analysis on request  
...
void PatExtractor2::analyze(const edm::Event& event, const edm::EventSetup& setup)
{
  ...
  if (do_fill_) 
  {
    PatExtractor2::fillInfo(&event); // Fill the ROOTuple
    PatExtractor2::doAna();          // Then do the analysis on request    
  }
...
void PatExtractor2::fillInfo(const edm::Event *event) 
{
  ...
  if (do_Jet_)      m_jet->writeInfo(event,m_MC,do_MC_);
...
void PatExtractor2::initialize() 
{
  ...
  m_jet      = new JetExtractor(do_Jet_,jet_tag_);
